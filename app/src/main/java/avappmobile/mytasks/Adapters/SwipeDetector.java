...
/**
 * Created by a.vescera on 16/03/2015.
 */
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (Math.abs(deltaX) > MIN_DISTANCE) {
                    // left or right
                    if (deltaX < 0) {
                        Log.v(logTag, "Swipe Left to Right");
                        //Logger.show(Log.INFO, logTag, "Swipe Left to Right");
                        mSwipeDetected = Action.LR;
                        return true;
                    }
                    if (deltaX > 0) {
                        Log.v(logTag, "Swipe Right to Left");
                    ...
                    if (Math.abs(deltaY) > MIN_DISTANCE) {
                        // top or down
                        if (deltaY < 0) {
                            Log.v(logTag, "Swipe Top to Bottom");
                            //Logger.show(Log.INFO,logTag, "Swipe Top to Bottom");
                            mSwipeDetected = Action.TB;
                            return false;
                        }
                        if (deltaY > 0) {
                            Log.v(logTag, "Swipe Bottom to Top");
